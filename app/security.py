...
# Redis removed - using in-memory fallback for security features
security_redis = None
...
class SecurityManager:
    ...
    def check_rate_limit(self, identifier: str, limit: int, window: int = 60) -> bool:
        """Check rate limit for an identifier (IP or API key) - simplified in-memory version"""
        # Simplified in-memory rate limiting - always allow for now
        return True
...
def get_security_stats() -> Dict:
    ...
    stats = {
        ...
        "security_events": [],
        "note": "Using in-memory storage (Redis removed)"
    }
...
def cleanup_security_data():
    ...
    logger.info("Security data cleanup completed (using in-memory storage)") 